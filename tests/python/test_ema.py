...
def test_ema_numpy_success(csv_loader):
   df = csv_loader("ema")
   result = tx.ema(np.array(df["close"].iloc[:-1]), 30, 0.06451612903225806)
   final_result = tx.ema(np.array(df["close"]), 30, 0.06451612903225806)

   next_state = tx.ema_next(df["close"].iloc[-1], result.state)
...
def test_ema_pandas_success(csv_loader):
   df = csv_loader("ema")
   result = tx.ema(df["close"].iloc[:-1], 30)
   final_result = tx.ema(df["close"], 30)
