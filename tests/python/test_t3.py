...
def test_t3_numpy_success(csv_loader):
    df = csv_loader("t3")
    result = tx.t3(np.array(df["close"].iloc[:-1]), 20)
    final_result = tx.t3(np.array(df["close"]), 20)

    next_state = tx.t3_next(df["close"].iloc[-1], result.state)
...
def test_t3_pandas_success(csv_loader):
    df = csv_loader("t3")
    result = tx.t3(df["close"].iloc[:-1], 20)
    final_result = tx.t3(df["close"], 20)
...
def test_thread_t3(thread_test):
    def t3_tx_lambda(data):
        return tx.t3(data, 20, release_gil = True)
