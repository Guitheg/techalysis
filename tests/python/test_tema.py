...
def test_tema_numpy_success(csv_loader):
    df = csv_loader("tema")
    result = tb.tema(np.array(df["close"].iloc[:-1]), 30, 0.06451612903225806)
    final_result = tb.tema(np.array(df["close"]), 30, 0.06451612903225806)

    next_state = tb.tema_next(df["close"].iloc[-1], result.state)
...
def test_tema_pandas_success(csv_loader):
    df = csv_loader("tema")
    result = tb.tema(df["close"].iloc[:-1], 30)
    final_result = tb.tema(df["close"], 30)
